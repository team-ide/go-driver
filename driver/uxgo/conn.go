...
import (
	...
	"github.com/team-ide/go-driver/driver/uxgo/scram"
	"io"
	"net"
	"os"
	"os/user"
...
// Common error types
var (
	ErrNotSupported              = errors.New("ux: Unsupported command")
	ErrInFailedTransaction       = errors.New("ux: Could not complete operation in a failed transaction")
	ErrSSLNotSupported           = errors.New("ux: SSL is not enabled on the server")
	ErrSSLKeyHasWorldPermissions = errors.New("ux: Private key file has group or world access. Permissions should be u=rw (0600) or less")
	ErrCouldNotDetectUsername    = errors.New("ux: Could not detect default username. Please provide one explicitly")
...
// Driver is the UXres database driver.
type Driver struct{}
...
// how ux creates network connections.
...
// scanner implements a tokenizer for libux-style option strings.
type scanner struct {
	s []rune
	i int
}
...
//
// The parsing code is based on conninfo_parse from libux's fe-connect.c
func parseOpts(name string, o values) error {
	...
	for {
		...
		if r, ok = s.SkipSpaces(); !ok {
			// If we reach the end here, the last value is just an empty string as per libux.
			o[string(keyRunes)] = ""
			break
		}
...
func (cn *conn) auth(r *readBuf, o values) {
	...
	case 7: // GSSAPI, startup
		if newGss == nil {
			errorf("kerberos error: no GSSAPI provider registered (import /ux/auth/kerberos if you need Kerberos support)")
		}
		cli, err := newGss()
		if err != nil {
...
func (st *stmt) exec(v []driver.Value) {
	if len(v) >= 65536 {
		errorf("got %d parameters but UXSQL only supports 65535 parameters", len(v))
	}
...
//	tblname := "my_table"
//	data := "my_data"
//	quoted := ux.QuoteIdentifier(tblname)
...
//
//	exp_date := ux.QuoteLiteral("2023-01-05 15:00:00Z")
//	err := db.Exec(fmt.Sprintf("CREATE ROLE my_user VALID UNTIL %s", exp_date))
//
// Any single quotes in name will be escaped. Any backslashes (i.e. "\") will be
// replaced by two backslashes (i.e. "\\") and the C-style escape identifier
// that UXSQL provides ('E') will be prepended to the string.
func QuoteLiteral(literal string) string {
	// This follows the UXSQL internal algorithm for handling quoted literals
	// from libux, which can be found in the "PQEscapeStringInternal" function,
	// which is found in the libux/fe-exec.c source file:
	// https://git.postgresql.org/gitweb/?p=postgresql.git;a=blob;f=src/interfaces/libux/fe-exec.c
	//
	// substitute any single-quotes (') with two single-quotes ('')
	literal = strings.Replace(literal, `'`, `''`, -1)
	// determine if the string has any backslashes (\) in it.
	// if it does, replace any backslashes (\) with two backslashes (\\)
	// then, we need to wrap the entire string with a UXSQL
...
func (cn *conn) sendBinaryModeQuery(query string, args []driver.Value) {
	if len(args) >= 65536 {
		errorf("got %d parameters but UXSQL only supports 65535 parameters", len(args))
	}
...
// parseEnviron tries to mimic some of libux's environment handling
//
...
func parseEnviron(env []string) (out map[string]string) {
	...
	for _, v := range env {
		...
		// UXSQL 2.1 manual. Unsupported but well-defined
		// keys cause a panic; these should be unset prior to
		// execution. Options which ux expects to be set to a
...
func isUTF8(name string) bool {
	// Recognize all sorts of silly things as "UTF-8", like UXres does
